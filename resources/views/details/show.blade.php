    ...
    <div class="p-6 text-gray-900 dark:text-gray-100 grid place-items-center">
        <div class="bg-white dark:bg-slate-800 p-8 rounded-lg shadow-md max-w-lg w-full">
            ...
            <div class="relative">
                ...
                <!-- Profile Photo -->
                <img src="https://picsum.photos/200/200?random={{ $id->id }}"
                     alt="{{ $id->name }} profile image"
            ...
            <div class="flex items-center mt-4">
                <h2 class="text-xl font-bold text-gray-800 dark:text-white" style="max-width: 10ch">{{ $id->name }}
                </h2>
            ...
            <!-- Bio -->
            <p class="text-gray-700 dark:text-white mt-2"> {{ $id->email }} </p>
            <!-- Asigmar roles solo el administrador lo puede ver -->
            @can('admin.users.edit')
                <div class="flex items-center mt-4 space-x-4">
                    <a href="{{ route('admin.users.edit', $id->id) }}" class="text-blue-500 hover:underline"> Asigmacion
                        de
                        roles
                    </a>
                </div>
            ...
            <div class="flex justify-between text-gray-600 mx-2">
                <div class="text-center">
                    <span class="text-gray-600 dark:text-gray-300 block font-bold text-sm"
                          style="max-width: 10ch">{{ $id->admission_date }}</span>
                    <span class="text-gray-600 dark:text-gray-300 text-xs">Fecha de ingreso</span>
                </div>
                <div class="text-center">
<span class="text-gray-600 dark:text-gray-300 block font-bold text-sm" style="max-width: 10ch">
@php
    $startVacation = new \DateTime($id->start_vacation);
    $endVacation = new \DateTime($id->end_vacation);
    $currentDate = new \DateTime();
    $diffEndVacation = $currentDate->diff($endVacation);
    $admissionDate = new \DateTime($id->admission_date);
...
@endphp
    ...
    @endif
                    ...
                    <span class="text-gray-600 dark:text-gray-300 text-xs">Dias disponibles</span>
                </div>
                <div class="text-center">
                    <span class="text-gray-600 dark:text-gray-300 block font-bold text-sm"
                          style="max-width: 10ch">{{ $id->id }}</span>
                    <span class="text-gray-600 dark:text-gray-300 text-xs">No. Nomina</span>
                </div>
            </div>
            <br>
            <!-- Vacations group -->
            <form action="{{ route('vacations.update', $id->id) }}" method="POST" class="flex flex-col">
                @csrf
